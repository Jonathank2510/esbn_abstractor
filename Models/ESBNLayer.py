...
class ESBNLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        for t in tf.range(len(inputs[1])):
            ...
            # Controller
            lstm_out, (hidden, cell_state) = self.lstm_cell(tf.squeeze(key_r, 1), states=[hidden, cell_state], training=True)
            # Generate write key
            ...
            if t > 0:
                ...
                w_k = tf.keras.activations.softmax(dot_product)
                c_k = tf.keras.activation.sigmoid(dot_product * self.confidence_gain + self.confidence_bias)
            ...
            # Write to memory
            M_k.write(t, key_w).mark_used()
            M_v.write(t, tf.squeeze(x, 1)).mark_used()
