...
class Dist3Transformer(tf.keras.Model):
    def __init__(self):
        ...
        self.n_layers = 1
        self.enc_layers = [
            EncoderLayer(d_model=self.z_size,
                         num_heads=self.n_heads,
                         dff=self.dim_ff)
            for _ in range(self.n_layers)]

        self.out_hidden = layers.Dense(256)
    ...
    def call(self, x_seq):
        ...
        z_seq = z_seq + pos_encoding[tf.newaxis, :length, :]

        for i in range(self.n_layers):
            z_seq = self.enc_layers[i](z_seq)

        tf.print("before mean")
        tf.print(z_seq.shape)
        tf.print(z_seq)
        # Average over transformed sequences
        z_seq = tf.reduce_mean(z_seq, axis=1)
        tf.print("reduce_mean")
        tf.print(z_seq.shape)
        tf.print(z_seq)

        out_hidden = self.out_hidden(z_seq)
        logits = self.y_out(out_hidden)
        return logits
